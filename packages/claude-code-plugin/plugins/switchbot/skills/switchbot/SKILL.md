...
Claude Code configures the SwitchBot MCP server automatically via `.mcp.json`.
No manual network configuration is required.

The MCP server (`switchbot mcp serve --tools all`) needs outbound HTTPS to `api.switch-bot.com`.
If the server reports connection errors, see `references/claude-code-network.md`.
